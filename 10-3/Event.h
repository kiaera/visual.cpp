...
public:
    ...
    // Event 번호 설정하기
    void Set_Event_No(int evtNo)
    {
        event_no = evtNo;
    }

    // Event priority 설정하기
    void Set_Event_Pri(int pri)
    {
        event_pri = pri;
    }

    // Event 이름 설정하기
    void Set_Event_Name(std::string nm)
    {
        event_name = nm;
    }

    // Event handler 주소 설정하기
    void Set_Event_Handler_Addr(int evtHndlerAddr)
    {
        event_handler_addr = evtHndlerAddr;
    }

    // Event generator 주소 설정하기
    void Set_Event_Gen_Addr(int genAddr)
    {
        event_gen_addr = genAddr;
    }

    // Event 상태 설정하기
    void Set_Event_Status(EventStatus evtStatus)
    {
